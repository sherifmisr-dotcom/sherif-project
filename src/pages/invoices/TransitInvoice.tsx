...
export default function TransitInvoice() {
  ...
  // Watch for customs_no changes and fetch customs fee
  useEffect(() => {
    const subscription = watch((value, { name }) => {
      if (name === 'customs_no' && value.customs_no && value.customs_no.trim()) {
        fetchCustomsFee(value.customs_no);
      } else if (name === 'customs_no' && !value.customs_no) {
        setCustomsFeeAmount(null);
      }
    });
    return () => subscription.unsubscribe();
  }, [watch]);

  const fetchCustomsFee = async (customsNo: string) => {
    try {
      // Check if this customs number is already used by another invoice
      const existingInvoices = await apiClient.getInvoices({ q: customsNo, limit: 1 });
      const existingInvoice = existingInvoices.data?.find(
        (inv: any) => inv.customsNo === customsNo && (!editingInvoice || inv.id !== editingInvoice.id)
      );
      if (existingInvoice) {
        setCustomsFeeAmount(null);
        showWarning(`رقم البيان ${customsNo} مرتبط بفاتورة سابقة (${existingInvoice.code})، لن يتم استيراد الرسوم الجمركية`);
        return;
      }

      const data = await apiClient.getCustomsFeeByDeclarationNo(customsNo);
      if (data && data.amount) {
        setCustomsFeeAmount(Number(data.amount));
      } else {
        setCustomsFeeAmount(null);
      }
    } catch (error) {
  ...
  const loadCustomers = async () => {
    try {
      const response = await apiClient.getCustomers({ type: 'TRANSIT', limit: 0 });
      if (response.data) {
    ...
    setItems((prevItems) => {
      return prevItems.map((item) => {
        if (item.id === id) {
          ...
          // Auto-fill customs fee amount when description is 'رسوم جمركية'
          if (field === 'description' && value === 'رسوم جمركية' && customsFeeAmount !== null) {
            updated.unitPrice = customsFeeAmount;
            const subtotal = customsFeeAmount * Number(updated.quantity);
            const vatAmount = subtotal * (Number(updated.vatRate) / 100);
            updated.amount = subtotal + vatAmount;

            // Show success message
            showSuccess(`تم استيراد الرسوم الجمركية تلقائياً: ${customsFeeAmount.toFixed(2)} ريال`);
          }
  ...
  const calculateTotals = () => {
    ...
    // Calculate clearance fees if auto mode is enabled
    const clearanceFees = autoClearanceFees && invoiceTotalAmount > 0
      ? Math.max(0, invoiceTotalAmount - itemsSubtotal)
      : 0;
  ...
  const onSubmit = async (data: InvoiceFormData) => {
    try {
      ...
      const { subtotal, clearanceFees } = calculateTotals();
      ...
      const invoiceData: any = {
        ...
        items: items
          .filter((item, index) => {
            ...
            if (autoClearanceFees && index === 0) {
              return clearanceFees > 0;
            }
            return item.description && item.amount > 0;
          })
          .map((item, index) => {
            // For clearance row, use calculated clearance fees as the amount
            if (autoClearanceFees && item.description === 'اجور تخليص') {
              return {
                description: item.description,
                unitPrice: clearanceFees,
                quantity: 1,
                vatRate: 0,
  ...
  const { subtotal, vatAmount, total, clearanceFees } = calculateTotals();
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 px-3 py-2 md:px-6 md:py-3 flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
        <Breadcrumb
          items={[
            ...
            { label: 'فواتير الترانزيت' }
          ]}
        />
        {canCreate('invoices.type3') && (
          <button
            onClick={() => {
              setEditingInvoice(null);
              reset({
            ...
            className="flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-6 md:py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors text-sm md:text-base w-full md:w-auto justify-center"
          >
            ...
            <div className="overflow-y-auto flex-1 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent [scrollbar-gutter:stable] hover:scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-600">
              <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
                ...
                <div>
                  ...
                  {/* Auto Clearance Fees Toggle - Compact inline switch */}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <label className="inline-flex items-center gap-2 cursor-pointer select-none">
                        <div className="relative">
                          <input
                            type="checkbox"
                            checked={autoClearanceFees}
                            onChange={(e) => {
                              const checked = e.target.checked;
                              setAutoClearanceFees(checked);
                              if (checked) {
                                // Set first item as clearance fees
                                setItems(prev => {
                                  const updated = [...prev];
                                  updated[0] = { ...updated[0], description: 'اجور تخليص', vatRate: 0 };
                                  return updated;
                                });
                              } else {
                                setInvoiceTotalAmount(0);
                                ...
                                setItems(prev => {
                                  const updated = [...prev];
                                  updated[0] = { ...updated[0], description: '', unitPrice: 0, amount: 0 };
                                  return updated;
                                });
                              }
                            }}
                          ...
                          />
                          <div className="w-9 h-5 bg-gray-300 dark:bg-gray-600 peer-checked:bg-amber-500 rounded-full transition-colors"></div>
                        ...
                        <span className={`text-xs font-medium ${autoClearanceFees ? 'text-amber-700 dark:text-amber-400' : 'text-gray-500 dark:text-gray-400'}`}>
                          حساب أجور التخليص تلقائياً
                        </span>
                      </label>
                    </div>
                    <button
                  ...
                  <div className="space-y-3">
                    {items.map((item, itemIndex) => {
                      const isClearanceRow = autoClearanceFees && itemIndex === 0;
                      return (
                        <div key={item.id}>
                          {/* Desktop: Grid layout */}
                          <div className={`hidden md:grid grid-cols-12 gap-2 ${isClearanceRow ? 'bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-400 dark:border-amber-600 rounded-xl p-2' : ''}`}>
                            {/* Description */}
                            <div className="col-span-3">
                              {isClearanceRow ? (
                                <div className="w-full px-3 py-2 border border-amber-400 dark:border-amber-600 rounded-lg bg-amber-100 dark:bg-amber-900/30 text-sm font-bold text-amber-800 dark:text-amber-300 flex items-center gap-2">
                                ...
                                <AutocompleteInput
                                  ...
                                  onSearch={async (query) => {
                                    ...
                                    return results.map((r: any) => r.description).filter((d: string) => !usedDescriptions.includes(d) && !(autoClearanceFees && d.includes('تخليص')));
                                  }}
                            ...
                            <div className="col-span-2">
                              <input
                                type="number"
                                placeholder={isClearanceRow ? 'الإجمالي المعلوم' : 'سعر الوحدة'}
                                value={isClearanceRow ? (invoiceTotalAmount || '') : (item.unitPrice || '')}
                                onChange={(e) => {
                                  if (isClearanceRow) {
                                    setInvoiceTotalAmount(parseFloat(e.target.value) || 0);
                                  } else {
                                    updateItem(item.id, 'unitPrice', parseFloat(e.target.value) || 0);
                                  }
                                }}
                                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 dark:text-white text-sm ${isClearanceRow
                                  ? 'border-amber-400 dark:border-amber-600 bg-white dark:bg-gray-700 font-bold text-amber-800 dark:text-amber-300'
                            ...
                            <div className="col-span-2">
                              <input
                                ...
                                max="100"
                                step="0.01"
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-300 text-sm font-medium cursor-not-allowed"
                              />
                            </div>
                            ...
                            <div className="col-span-2">
                              <input
                                type="text"
                                value={isClearanceRow ? clearanceFees.toFixed(2) : item.amount.toFixed(2)}
                                readOnly
                                className={`w-full px-3 py-2 border rounded-lg text-sm font-medium text-center ${isClearanceRow
                                  ? 'border-amber-400 dark:border-amber-600 bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 font-bold'
                                  : 'border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-300'
                                  }`}
                              />
                            </div>
                          ...
                          <div className="md:hidden bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 border border-gray-200 dark:border-gray-600 space-y-2">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              <div>
                                <label className="text-[10px] text-gray-500 dark:text-gray-400 mb-0.5 block">{isClearanceRow ? 'الإجمالي المعلوم' : 'سعر الوحدة'}</label>
                                <input
                                  type="number"
                                  placeholder={isClearanceRow ? 'الإجمالي' : 'سعر'}
                                  value={isClearanceRow ? (invoiceTotalAmount || '') : (item.unitPrice || '')}
                                  onChange={(e) => {
                                    if (isClearanceRow) {
                                      setInvoiceTotalAmount(parseFloat(e.target.value) || 0);
                                  ...
                                  className={`w-full px-2 py-1.5 border rounded-lg focus:ring-2 focus:ring-blue-500 dark:text-white text-xs ${isClearanceRow
                                    ? 'border-amber-400 dark:border-amber-600 bg-white dark:bg-gray-700 font-bold text-amber-800 dark:text-amber-300'
                                    : 'border-gray-300 dark:border-gray-600 dark:bg-gray-700'
                                    }`}
                                />
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  min="1"
                                  step="0.01"
                                  className="w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white text-xs"
                                />
                              </div>
                              <div>
                                ...
                                <input
                                  ...
                                  value={applyVAT ? (item.vatRate || '') : 0}
                                  readOnly
                                  className="w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-300 text-xs cursor-not-allowed"
                                />
                              </div>
                            </div>
                            {/* Total + Delete */}
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] text-gray-500 dark:text-gray-400">{isClearanceRow ? 'أجور التخليص:' : 'الإجمالي:'}</span>
                                <span className={`text-sm font-bold ${isClearanceRow ? 'text-amber-800 dark:text-amber-300' : 'text-gray-900 dark:text-white'}`}>{isClearanceRow ? clearanceFees.toFixed(2) : item.amount.toFixed(2)}</span>
                              </div>
                              {!isClearanceRow && (
                                <button
                                  type="button"
