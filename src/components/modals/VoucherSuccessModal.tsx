...
export default function VoucherSuccessModal({
    ...
    const title = isReceipt ? 'تم إنشاء سند القبض بنجاح' : 'تم إنشاء سند الصرف بنجاح';
    const codeLabel = isReceipt ? 'رقم سند القبض' : 'رقم سند الصرف';
    const iconColor = isReceipt ? 'from-green-400 to-green-600' : 'from-red-400 to-red-600';
    const iconGlow = isReceipt ? 'bg-green-400' : 'bg-red-400';
                ...
                <div className="px-8 pb-8 pt-4 text-center">
                    <div className="flex justify-center mb-4">
                        <div className="relative">
                            <div className={`absolute inset-0 ${iconGlow} rounded-full blur-xl opacity-50 animate-pulse`}></div>
                            <div className={`relative bg-gradient-to-br ${iconColor} rounded-full p-4`}>
                    ...
                    <p className={`text-xl font-bold mb-6 ${isReceipt ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                        {voucherCode}
                    </p>
