...
interface TreasuryReportPrintProps {
    startDate: string;
    endDate: string;
...
export default function TreasuryReportPrint({
    startDate,
    endDate,
    transactions,
    openingBalance,
    openingBalanceDescription = 'رصيد أول المدة',
    totalDebit,
    totalCredit,
    closingBalance,
    onClose,
}: TreasuryReportPrintProps) {
    const handlePrint = () => {
        const printContent = document.getElementById('treasury-report-print-content');
        ...
        <ModalOverlay>
            <div className="bg-[#eef2f5] rounded-lg max-w-[210mm] max-h-[95vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex justify-between items-center print:hidden z-10">
                    <h3 className="text-lg font-bold text-gray-900">معاينة تقرير حركة الخزنة</h3>
                ...
                {/* Print Content */}
                <div id="treasury-report-print-content" className="bg-white p-8">
                    <div className="max-w-full">
                        {/* Title & Date Range */}
                        <div className="flex justify-between items-start mb-6 pb-4 border-b-2 border-slate-800">
                            <div>
                                <h2 className="text-2xl font-bold text-blue-700 mb-1">كشف حركة الخزنة</h2>
                            ...
                            <div className="text-left flex flex-col items-center mt-1">
                                <div className="text-lg font-bold text-blue-700 mb-1 uppercase tracking-wider">
                        ...
                        <div className="grid grid-cols-4 gap-4 mb-8">
                            ...
                            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col justify-center relative overflow-hidden">
                                ...
                                <p className="text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">{openingBalanceDescription}</p>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    <tr className="bg-gray-50">
                                        ...
                                        <td className="px-4 py-3 text-sm font-medium text-gray-900">{openingBalanceDescription}</td>
                                    ...
                                    {transactions.map((tx) => (
                                        <tr key={tx.id} className="hover:bg-gray-50">
                                            <td className="px-4 py-3 text-sm font-medium text-blue-600">{tx.voucher?.code || '-'}</td>
                                            <td className="px-4 py-3 text-sm text-gray-600">
                                            ...
                                            <td className="px-4 py-3 text-sm text-gray-900">
                                                {tx.voucher?.type === 'INTERNAL_TRANSFER'
                                                    ...
                                                    : (
                                                        <>
                                                            {tx.voucher?.note || tx.note || ''}
                                                            {tx.voucher?.partyName && <span className="text-gray-500"> - {tx.voucher.partyName}</span>}
                                                        </>
                                                    )
                                                }
                                            </td>
